...
//Gets the tasks for the current week, if there's a current week
  ...
  dbCalls.getGroupInfo(groupId, next)
    .then((response) => {
      
      //Check the requester is a group member
      const isMember = businessLogic.checkMembership( req.payload._id, response.members );
      if (!isMember){ 
        res.status(401).json({message: "Sorry, you're not authorized to perform this action."}) 
        return;
      }
